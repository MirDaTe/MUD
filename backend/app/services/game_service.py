...
from .admin_service import is_admin, admin_set_stat, admin_teleport, admin_give_item, get_all_users, get_all_characters, admin_list_rooms
...
from .inventory_service import sell_item, dismantle_item, discard_item, equip_status, unequip
...
def execute_command(db, char, cmd):
 ...
 문파:{fn}  위치:#{char.current_room_id}{buf}"""
    ...
    if v == "equip":
        ...
        if it.item_type not in ("weapon","armor","accessory"): return [{"type":"system","content":"장착 불가.","style":"warning"}]
        inv.equipped=1; s=it.stats or {}
        char.attack+=s.get("attack",0); char.defense+=s.get("defense",0)
        char.max_hp+=s.get("hp",0); char.max_mp+=s.get("mp",0); char.speed+=s.get("speed",0)
        db.commit(); return [{"type":"system","content":f"{it.name} 장착!","style":"normal"}]

    if v == "unequip": e=unequip(db,char,rest); return [{"type":"system","content":e if e else f"{rest} 해제.","style":"warning" if e else "normal"}]
    if v == "eq":
        es=equip_status(db,char)
        lines=["[장비현황]"]
        for sl,info in es["slots"].items(): lines.append(f"  {sl}: {info['name'] if info else '빈칸'}")
        lines.append(f"  총공:{es['total_attack']} 방:{es['total_defense']} HP:{es['max_hp']} MP:{es['max_mp']} 속:{es['speed']}")
        return [{"type":"system","content":"\n".join(lines),"style":"normal"}]
    ...
    if v == "admin_rooms":
        ...
        for r in rooms[:30]:
            ...
            lines.append(f"  #{r['id']} {r['name']} ({r['region']})→{ex}")
        return [{"type":"system","content":"\n".join(lines),"style":"normal"}]
...
[기본]
 look/보기      주변보기
 북/남/동/서/위/아래   이동
 attack 대상/공격  cast 무공/시전
 meditate/수련   talk 대상/대화
 status/상태     inv/가방

[장비/아이템]
 equip/장착  unequip/해제  eq/장비
 use/사용    sell/판매     dismantle/분해
 discard/버리기  shop/상점  buy/구매
 gold/돈/은전
...
[길드/문파]
 guild_create/길드생성  guild_join/길드가입
 guild_approve/승인     guild_leave/탈퇴
 guild_kick/추방       guild_rank/계급
 guild_info/guild/길드  guild_storage/길드창고
 guild_deposit/보관     guild_withdraw/인출
 guild_gold_add/입금    guild_gold_out/출금

[기타]
 quest/퀘스트  save/저장  help/도움
 admin_set/tp/give/users/chars/rooms/god""","style":"normal"}]
